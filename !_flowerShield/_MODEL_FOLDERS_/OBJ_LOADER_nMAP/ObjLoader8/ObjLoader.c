...
#define VEC_SIZE 0.2f
void DrawTangentsAndStuff(ObjMesh *pMesh)
{
	...
	for( ; e != v; ++v,++n,++b,++t )
	{
		glColor3f(1,0,0);
		glVertex3fv( (float*) v );
		glVertex3f(v->x+t->x*VEC_SIZE,v->y+t->y*VEC_SIZE,v->z+t->z*VEC_SIZE);

		glColor3f(0,1,0);
		glVertex3fv( (float*) v );
		glVertex3f(v->x+n->x*VEC_SIZE,v->y+n->y*VEC_SIZE,v->z+n->z*VEC_SIZE);

		glColor3f(0,0,1);
		glVertex3fv( (float*) v );
		glVertex3f(v->x+b->x*VEC_SIZE,v->y+b->y*VEC_SIZE,v->z+b->z*VEC_SIZE);
	}
	glEnd();
			...
			case VERTEX_BUFFER_OBJECT:
				{
					...
					*/
					if(pMesh->m_aTexCoordArray)
					{
						glTexCoordPointer(2,GL_FLOAT,0, (void*)( 6 * pMesh->m_iVBO_Size * sizeof(float) ) );
						glEnableClientState(GL_TEXTURE_COORD_ARRAY);
					}
	...
	case VERTEX_ARRAY:
		{
			...
			*/
			...
			else 
			{
				...
				glBufferDataARB( GL_ARRAY_BUFFER_ARB, 8 * ActualVertexArraySize * sizeof(float), NULL, GL_STATIC_DRAW_ARB );
				...
				glBufferSubDataARB( GL_ARRAY_BUFFER_ARB, 6 * ActualVertexArraySize * sizeof(float), 2 * ActualVertexArraySize * sizeof(float), pMesh->m_aTexCoordArray );


				/* generate a vertex buffer object */
				glGenBuffersARB( 1, &pMesh->m_iVBO_ElemNum );		
				glBindBufferARB( GL_ELEMENT_ARRAY_BUFFER_ARB, pMesh->m_iVBO_ElemNum );
				glBufferDataARB( GL_ELEMENT_ARRAY_BUFFER_ARB, pMesh->m_iNumberOfFaces * sizeof(float), pMesh->m_aIndices, GL_STATIC_DRAW_ARB );

				/*
				*	Since the data has been uploaded to the graphics card, we may as well free the client side data
				*/
				if (pMesh->m_aTexCoordArray) {
					free(pMesh->m_aTexCoordArray);
					pMesh->m_aTexCoordArray =NULL;
